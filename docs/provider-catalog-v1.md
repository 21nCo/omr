...
Cursor filter sets use code point ordering, independent of host locale. Equivalent grants can continue pagination on another instance; changed grants still require a fresh discovery. Any cursor created before this ordering change may be rejected when its filter values sort differently, including valid action IDs with mixed case, punctuation, or digits. Clients should restart discovery when an older cursor is rejected. Manifest IDs, hashes, and catalog revisions retain their prior ordering and values.
...
If persisting that known-missing remote transition fails, discovery still omits the effective binding's tools for the current request; execution still returns `CONNECTION_UNAVAILABLE`. With no other ready binding, the provider reports `expired` for that response. Another ready binding can keep the provider state `ready`, even while the selected missing binding's tools stay hidden. The stored health transition is attempted again on a later request. A failed write emits a structured warning with the binding ID and error type but no credential or error message; a terminally unavailable binding has a distinct event. Unrelated remote lookup errors still propagate.
