...
If persisting that known-missing remote transition fails, discovery still omits its tools and reports it unavailable for the current request; execution still returns `CONNECTION_UNAVAILABLE`. The stored health transition is attempted again on a later request. Unrelated remote lookup errors still propagate.
